...
Interactive argument helper — select, validate, and export CLI arguments to your shell.
...
Installation
------------
Build from source with Go (default approach for now):

```bash
# from repository root
go build -o argonaut
# Windows:
# go build -o argonaut.exe
```
...
Invoke-Expression -Command (.\argonaut.exe bind \
  --flag=name \
...
```bash
# Example: produce exports and evaluate them in the current shell
eval "$(./argonaut bind \
  --flag=name \
  --flag-name-default=happy \
  --flag-name-choices=happy,people \
...
# Or using process substitution (if supported):
# source <(./argonaut bind --flag=... -- a --name=people)
